...
bool32 ShouldDoProfBirchRotomCatalogCall(void)
{
    if (FlagGet(FLAG_PROF_BIRCH_ROTOM_CALL))
    {
        switch (gMapHeader.mapType)
        {
            case MAP_TYPE_TOWN:
...
bool32 ShouldDoProfBirchNewStarterCall(void)
{
    if (FlagGet(FLAG_BIRCH_GIVE_NEW_STARTER_CALL) && !FlagGet(FLAG_PROF_BIRCH_ROTOM_CALL))
    {
        switch (gMapHeader.mapType)
        {
            ...
            case MAP_TYPE_ROUTE:
            case MAP_TYPE_OCEAN_ROUTE:
                if (++(*GetVarPointer(VAR_PROF_BIRCH_CALL_STEP_COUNTER)) < 250)
                {
                    return FALSE;
                }
                break;
            ...
            switch ((u8)gLinkPlayers[i].version)
            {
                ...
                case VERSION_EMERALD:
                    if (gLinkPlayers[i].gender == 0)
                        ...
                        #ifdef CHANGE_PLAYER_
                        linkSpriteId = OBJ_EVENT_GFX_RIVAL_LEAF_NORMAL;
                        #else
...
static void LoadLinkPartnerObjectEventSpritePalette(u16 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    if (graphicsId == OBJ_EVENT_GFX_LINK_RS_BRENDAN ||
        ...
        graphicsId == OBJ_EVENT_GFX_RIVAL_LEAF_NORMAL)
    #else
    if (graphicsId == OBJ_EVENT_GFX_LINK_RS_BRENDAN ||
        graphicsId == OBJ_EVENT_GFX_LINK_RS_MAY ||
    ...
    #endif
    {
        ...
        if (obj != OBJECT_EVENTS_COUNT)
        {
            ...
            case OBJ_EVENT_GFX_RIVAL_RED_NORMAL:
                LoadPalette(gObjectEventPal_Red2, 0x100 + (adjustedPaletteNum << 4), 0x20);
                break;
            case OBJ_EVENT_GFX_RIVAL_LEAF_NORMAL:
                LoadPalette(gObjectEventPal_Leaf2, 0x100 + (adjustedPaletteNum << 4), 0x20);
                break;
            #else
...
void PutZigzagoonInPlayerParty(void)
{
    ...
    u16 monData;
    u16 abilityNone;
    CreateMonSimple(&gPlayerParty[0], SPECIES_ZIGZAGOON, 7, USE_RANDOM_GENDER, USE_RANDOM_NATURE, USE_RANDOM_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
    monData = MOVE_TACKLE;
...
void ShowScrollableMultichoice(void)
{
    ...
    switch (gSpecialVar_0x8004)
    {
        ...
        case SCROLL_MULTI_BATTLE_TENT_RULES:
            ...
            task->tTaskId = taskId;
            break;
        case SCROLL_MULTI_GAME_CORNER_POKEMONS:
            task->tMaxItemsOnScreen = MAX_SCROLL_MULTI_ON_SCREEN;
            task->tNumItems = 12;
            task->tLeft = 14;
            task->tTop = 1;
            ...
            task->tTaskId = taskId;
            break;
        case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE:
            task->tMaxItemsOnScreen = MAX_SCROLL_MULTI_ON_SCREEN;
            task->tNumItems = 16;
            task->tLeft = 14;
            task->tTop = 1;
            ...
            task->tTaskId = taskId;
            break;
        case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_BIRD_FISH:
            task->tMaxItemsOnScreen = MAX_SCROLL_MULTI_ON_SCREEN;
            task->tNumItems = 3;
            task->tLeft = 14;
            task->tTop = 1;
            ...
            task->tTaskId = taskId;
            break;
        case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_DRAKE_DINO:
            task->tMaxItemsOnScreen = MAX_SCROLL_MULTI_ON_SCREEN;
            task->tNumItems = 3;
            task->tLeft = 14;
            task->tTop = 1;
        ...
        case SCROLL_MULTI_FORTREE_SCHOOL:
            ...
            task->tTaskId = taskId;
            break;
        case SCROLL_MULTI_FORTREE_SCHOOL2:
            task->tMaxItemsOnScreen = MAX_SCROLL_MULTI_ON_SCREEN;
            task->tNumItems = 10;
            task->tLeft = 14;
            task->tTop = 1;
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] = 
{
    ...
    [SCROLL_MULTI_GAME_CORNER_POKEMONS] = 
    {
        gText_Pokemon1AndPrice,
        gText_Pokemon2AndPrice,
        gText_Pokemon3AndPrice,
        gText_Pokemon4AndPrice,
        gText_Pokemon5AndPrice,
        gText_Pokemon6AndPrice,
        gText_Pokemon7AndPrice,
        gText_Pokemon8AndPrice,
        gText_Pokemon9AndPrice,
        gText_Pokemon10AndPrice,
        gText_Pokemon11AndPrice,
        gText_Exit
    },
    [SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE] = 
    {
        gText_Fossil1,
        gText_Fossil2,
        gText_Fossil3,
        ...
        gText_Fossil12, //Bird
        gText_Fossil13, //Fish
        gText_Fossil14, //Drake
        gText_Fossil15, //Dino
        gText_Exit
    },
    [SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_BIRD_FISH] = 
    {
        gText_Fossil14,
        gText_Fossil15,
        gText_Exit
    },
    [SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_DRAKE_DINO] = 
    {
        gText_Fossil12,
        gText_Fossil13,
        gText_Exit
    },
    [SCROLL_MULTI_PETALBURG_SCHOOL] = 
    {
        gText_WeatherHarshSunlight,
        gText_WeatherExtremeHarshSunlight,
        gText_WeatherRain,
        gText_WeatherHeavyRain,
        gText_WeatherSandstorm,
    ...
    {
        gText_TerrainElectricTerrain,
        gText_TerrainGrassyTerrain,
        gText_TerrainMistyTerrain,
        gText_TerrainPsychicTerrain,
        gText_TerrainFrozenTerrain,
        gText_TerrainCursedTerrain,
        gText_TerrainRavineTerrain,
        gText_TerrainBurningTerrain,
    ...
    {
        gText_ClassAcid,
        gText_ClassAirbourneStyle,
        gText_ClassBallistic,
        gText_ClassBiting,
        gText_ClassCrash,
        gText_ClassDance,
        gText_ClassFang,
        gText_ClassField,
        gText_ClassHitFlyingType,
        gText_ClassHitMinimize,
        gText_ClassHitSemiInvulnerable,
        gText_ClassIgnoreAbilities,
        gText_ClassIgnoreDefense,
        gText_ClassIgnoreEvasion,
        gText_ClassLauncher,
        gText_ClassMagnet,
    },
    [SCROLL_MULTI_FORTREE_SCHOOL2] = 
    {
        gText_ClassMud,
        gText_ClassPowder,
        gText_ClassPunching,
        ...
        gText_ClassThaw,
        gText_ClassWave,
        gText_ClassWeatherRelated,
        gText_ClassWind,
        gText_ClassWing
    },
    [SCROLL_MULTI_BLACKSMITH] = 
    {
        gText_Masamune,
        gText_Muramasa,
        gText_Fireblender,
...
void UpdateSlateportPointsWindow(void)
{
    u8 string[32];
    u32 x;
    StringCopy(ConvertIntToDecimalStringN(string, gSaveBlock2Ptr->frontier.slateportTentPoints, STR_CONV_MODE_RIGHT_ALIGN, 4), gText_SP);
    x = GetStringRightAlignXOffset(1, string, 48);
    AddTextPrinterParameterized(sBattlePointsWindowId, 1, string, x, 1, 0, NULL);
}

void ShowSlateportPointsWindow(void)
{
    static const struct WindowTemplate sBattlePoints_WindowTemplate = {
        .bg = 0,
        .tilemapLeft = 1,
    ...
    sBattlePointsWindowId = AddWindow(&sBattlePoints_WindowTemplate);
    SetStandardWindowBorderStyle(sBattlePointsWindowId, 0);
    UpdateSlateportPointsWindow();
    CopyWindowToVram(sBattlePointsWindowId, 2);
}

void UpdateVerdanturfPointsWindow(void)
{
    u8 string[32];
    u32 x;
    StringCopy(ConvertIntToDecimalStringN(string, gSaveBlock2Ptr->frontier.verdanturfTentPoints, STR_CONV_MODE_RIGHT_ALIGN, 4), gText_VP);
    x = GetStringRightAlignXOffset(1, string, 48);
    AddTextPrinterParameterized(sBattlePointsWindowId, 1, string, x, 1, 0, NULL);
}

void ShowVerdanturfPointsWindow(void)
{
    static const struct WindowTemplate sBattlePoints_WindowTemplate = {
        .bg = 0,
        .tilemapLeft = 1,
    ...
    sBattlePointsWindowId = AddWindow(&sBattlePoints_WindowTemplate);
    SetStandardWindowBorderStyle(sBattlePointsWindowId, 0);
    UpdateVerdanturfPointsWindow();
    CopyWindowToVram(sBattlePointsWindowId, 2);
}

void UpdateFallarborPointsWindow(void)
{
    u8 string[32];
    u32 x;
    StringCopy(ConvertIntToDecimalStringN(string, gSaveBlock2Ptr->frontier.fallarborTentPoints, STR_CONV_MODE_RIGHT_ALIGN, 4), gText_FP);
    x = GetStringRightAlignXOffset(1, string, 48);
    AddTextPrinterParameterized(sBattlePointsWindowId, 1, string, x, 1, 0, NULL);
}

void ShowFallarborPointsWindow(void)
{
    static const struct WindowTemplate sBattlePoints_WindowTemplate = {
        .bg = 0,
        .tilemapLeft = 1,
...
void TakeSlateportTentPoints(void)
{
    if (gSaveBlock2Ptr->frontier.slateportTentPoints < gSpecialVar_0x8004)
    {
        gSaveBlock2Ptr->frontier.slateportTentPoints = 0;
    }
    else
    {
        gSaveBlock2Ptr->frontier.slateportTentPoints -= gSpecialVar_0x8004;
    }
}

void TakeVerdanturfTentPoints(void)
{
    if (gSaveBlock2Ptr->frontier.verdanturfTentPoints < gSpecialVar_0x8004)
    {
        gSaveBlock2Ptr->frontier.verdanturfTentPoints = 0;
    }
    else
    {
        gSaveBlock2Ptr->frontier.verdanturfTentPoints -= gSpecialVar_0x8004;
    }
}

void TakeFallarborTentPoints(void)
{
    if (gSaveBlock2Ptr->frontier.fallarborTentPoints < gSpecialVar_0x8004)
    {
        gSaveBlock2Ptr->frontier.fallarborTentPoints = 0;
    }
    else
    {
        gSaveBlock2Ptr->frontier.fallarborTentPoints -= gSpecialVar_0x8004;
    }
}
...
u16 GetSlateportTentPoints(void)
{
    return gSaveBlock2Ptr->frontier.slateportTentPoints;
}

u16 GetVerdanturfTentPoints(void)
{
    return gSaveBlock2Ptr->frontier.verdanturfTentPoints;
}

u16 GetFallarborTentPoints(void)
{
    return gSaveBlock2Ptr->frontier.fallarborTentPoints;
}
...
static void FillFrontierExchangeCornerWindowAndItemIcon(u16 menu, u16 selection)
{
    ...
    if (menu >= SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1 && menu <= SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE)
    {
        ...
        switch (menu)
        {
            ...
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR:
                ...
                break;
            case SCROLL_MULTI_ST_EXCHANGE_CORNER_EVOLVE_STONE_VENDOR:
                AddTextPrinterParameterized2(0, 1, sFrontierExchangeCorner_EvolveStonesDescriptions[selection], 0, NULL, 2, 1, 3);
                ShowFrontierExchangeCornerItemIcon(sFrontierExchangeCorner_EvolveStones[selection]);
                break;
            case SCROLL_MULTI_VT_EXCHANGE_CORNER_EVOLVE_ITEM_VENDOR:
                AddTextPrinterParameterized2(0, 1, sFrontierExchangeCorner_EvolveItemsDescriptions[selection], 0, NULL, 2, 1, 3);
                ShowFrontierExchangeCornerItemIcon(sFrontierExchangeCorner_EvolveItems[selection]);
                break;
            case SCROLL_MULTI_FT_EXCHANGE_CORNER_EVOLVE_ITEM_VENDOR:
                AddTextPrinterParameterized2(0, 1, sFrontierExchangeCorner_EvolveItemsDescriptions2[selection], 0, NULL, 2, 1, 3);
                ShowFrontierExchangeCornerItemIcon(sFrontierExchangeCorner_EvolveItems2[selection]);
                break;
            case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE:
                AddTextPrinterParameterized2(0, 1, sFossilRegenerator_FossilsDescriptions[selection], 0, NULL, 2, 1, 3);
                ShowFrontierExchangeCornerItemIcon(sFossilRegenerator_Fossils[selection]);
                break;
            case SCROLL_MULTI_BLACKSMITH:
                AddTextPrinterParameterized2(0, 1, sBlacksmith_ItemsDescriptions[selection], 0, NULL, 2, 1, 3);
                ShowFrontierExchangeCornerItemIcon(sBlacksmith_Items[selection]);
                break;
        }
    }
    else if (menu >= SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_BIRD_FISH)
    {
        FillWindowPixelRect(0, PIXEL_FILL(1), 0, 0, 216, 32);
        switch (menu)
        {        
        case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_BIRD_FISH:
            AddTextPrinterParameterized2(0, 1, sFossilRegenerator_FossilBirdDescriptions[selection], 0, NULL, 2, 1, 3);
            ShowFrontierExchangeCornerItemIcon(sFossilRegenerator_FossilBird[selection]);
            break;
        case SCROLL_MULTI_FOSSIL_REGENERATOR_DEVICE_DRAKE_DINO:
            AddTextPrinterParameterized2(0, 1, sFossilRegenerator_FossilDrakeDescriptions[selection], 0, NULL, 2, 1, 3);
            ShowFrontierExchangeCornerItemIcon(sFossilRegenerator_FossilDrake[selection]);
            break;
        }
...
void TryPutACosplayOnPikachu(void)
{
    struct Pokemon *mon = &gPlayerParty[gSpecialVar_0x8004];   
    u16 baseSpecies = GetMonData(mon, MON_DATA_SPECIES);
    u16 targetSpecies = gSpecialVar_0x8005; //<-------| SPECIES_PIKACHU_*
    bool8 NeedToReplaceMove = TRUE;
    u8 i;

    switch (targetSpecies)
    {
        case SPECIES_PIKACHU_ROCK_STAR:
            gMoveToLearn = MOVE_METEOR_MASH;
            break;
        case SPECIES_PIKACHU_POP_STAR:
            gMoveToLearn = MOVE_DRAINING_KISS;
            break;
        case SPECIES_PIKACHU_BELLE:
            gMoveToLearn = MOVE_ICICLE_CRASH;
            break;
        case SPECIES_PIKACHU_PH_D:
            gMoveToLearn = MOVE_PSYSHOCK;
            break;
    ...
    gSpecialVar_0x8006 = gMoveToLearn;
    GetMonNickname(mon, gStringVar1);
    StringCopy(gStringVar2, gMoveNames[gMoveToLearn]);

    //Doesn't fit
    if (baseSpecies != SPECIES_PIKACHU_COSPLAY
     && baseSpecies != SPECIES_PIKACHU_ROCK_STAR
    ...
    //Already using the Cosplay
    else if (baseSpecies == targetSpecies)
    {
        gSpecialVar_Result = 1;
        return;
    }
    //Doesn't trust enough
    else if (GetMonData(mon, MON_DATA_FRIENDSHIP) < 0 /* 200 */)
    {
        gSpecialVar_Result = 2;
        return;
    }
    ...
    {                
        ...
        {
            if (GetMonData(mon, MON_DATA_MOVE1 + i, NULL) == MOVE_METEOR_MASH
             ...
             || GetMonData(mon, MON_DATA_MOVE1 + i, NULL) == MOVE_NONE)
            {
                ...
                NeedToReplaceMove = FALSE;
                gSpecialVar_Result = 3;
                break;
            }
        }

        if (NeedToReplaceMove)
        {
...
void RemovePikachuCosplay(void)
{
    struct Pokemon *mon = &gPlayerParty[gSpecialVar_0x8004];   
    u16 baseSpecies = GetMonData(mon, MON_DATA_SPECIES);
    u16 originalSpecies = SPECIES_PIKACHU_COSPLAY;
    u8 i,j;

    //Remove Cosplay
    if (baseSpecies == SPECIES_PIKACHU_ROCK_STAR
     || baseSpecies == SPECIES_PIKACHU_POP_STAR
     || baseSpecies == SPECIES_PIKACHU_BELLE
     || baseSpecies == SPECIES_PIKACHU_PH_D
     || baseSpecies == SPECIES_PIKACHU_LIBRE)
...
void TeachSpecialMove(void)
{
    struct Pokemon *mon = &gPlayerParty[gSpecialVar_0x8004];
    u16 i;

    SetMonMoveSlot(mon, gSpecialVar_0x8006, gSpecialVar_0x8005);
    RemoveMonPPBonus(mon, gSpecialVar_0x8005);
}

u8 GetDayOfWeek(void)
{
    gSpecialVar_Result = RtcGetLocalDayOfWeek();

    return gSpecialVar_Result; //0-sunday ~ 6-saturday
}

u8 CheckRayquazaInParty(void)
{
    u8 i;
    struct Pokemon *pokemon;
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(pokemon, MON_DATA_SANITY_HAS_SPECIES) && !GetMonData(pokemon, MON_DATA_IS_EGG))
        {
            if (GetMonData(pokemon, MON_DATA_SPECIES) == SPECIES_RAYQUAZA)
            {
                gSpecialVar_Result = TRUE;
                return;
            }
