...
EWRAM_DATA bool8 NuzlockeIsMapBlocked[PARTY_SIZE] = {0};
EWRAM_DATA bool8 NuzlockeIsSpeciesAlreadyCaught[PARTY_SIZE] = {0};
EWRAM_DATA bool8 NuzlockeIsEvoLineAlreadyCaught[PARTY_SIZE] = {0};
EWRAM_DATA bool8 OneTypeChallengeIsTypeBlocked[PARTY_SIZE] = {0};
...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_ROXANNE] = REMATCH(TRAINER_GYM_LEADER_ROXANNE_1, TRAINER_GYM_LEADER_ROXANNE_2, TRAINER_GYM_LEADER_ROXANNE_3, TRAINER_GYM_LEADER_ROXANNE_4, TRAINER_GYM_LEADER_ROXANNE_5, RUSTBORO_CITY),
    [REMATCH_BRAWLY] = REMATCH(TRAINER_GYM_LEADER_BRAWLY_1, TRAINER_GYM_LEADER_BRAWLY_2, TRAINER_GYM_LEADER_BRAWLY_3, TRAINER_GYM_LEADER_BRAWLY_4, TRAINER_GYM_LEADER_BRAWLY_5, DEWFORD_TOWN),
    [REMATCH_WATTSON] = REMATCH(TRAINER_GYM_LEADER_WATTSON_1, TRAINER_GYM_LEADER_WATTSON_2, TRAINER_GYM_LEADER_WATTSON_3, TRAINER_GYM_LEADER_WATTSON_4, TRAINER_GYM_LEADER_WATTSON_5, MAUVILLE_CITY),
    [REMATCH_FLANNERY] = REMATCH(TRAINER_GYM_LEADER_FLANNERY_1, TRAINER_GYM_LEADER_FLANNERY_2, TRAINER_GYM_LEADER_FLANNERY_3, TRAINER_GYM_LEADER_FLANNERY_4, TRAINER_GYM_LEADER_FLANNERY_5, LAVARIDGE_TOWN),
    [REMATCH_NORMAN] = REMATCH(TRAINER_GYM_LEADER_NORMAN_1, TRAINER_GYM_LEADER_NORMAN_2, TRAINER_GYM_LEADER_NORMAN_3, TRAINER_GYM_LEADER_NORMAN_4, TRAINER_GYM_LEADER_NORMAN_5, PETALBURG_CITY),
    [REMATCH_WINONA] = REMATCH(TRAINER_GYM_LEADER_WINONA_1, TRAINER_GYM_LEADER_WINONA_2, TRAINER_GYM_LEADER_WINONA_3, TRAINER_GYM_LEADER_WINONA_4, TRAINER_GYM_LEADER_WINONA_5, FORTREE_CITY),
    [REMATCH_TATE_AND_LIZA] = REMATCH(TRAINER_GYM_LEADER_TATE_AND_LIZA_1, TRAINER_GYM_LEADER_TATE_AND_LIZA_2, TRAINER_GYM_LEADER_TATE_AND_LIZA_3, TRAINER_GYM_LEADER_TATE_AND_LIZA_4, TRAINER_GYM_LEADER_TATE_AND_LIZA_5, MOSSDEEP_CITY),
    [REMATCH_JUAN] = REMATCH(TRAINER_GYM_LEADER_WALLACE_1, TRAINER_GYM_LEADER_WALLACE_2, TRAINER_GYM_LEADER_WALLACE_3, TRAINER_GYM_LEADER_WALLACE_4, TRAINER_GYM_LEADER_WALLACE_5, SOOTOPOLIS_CITY),
    [REMATCH_SIDNEY] = REMATCH(TRAINER_ELITE_FOUR_SIDNEY, TRAINER_ELITE_FOUR_SIDNEY, TRAINER_ELITE_FOUR_SIDNEY, TRAINER_ELITE_FOUR_SIDNEY, TRAINER_ELITE_FOUR_SIDNEY, EVER_GRANDE_CITY),
    [REMATCH_PHOEBE] = REMATCH(TRAINER_ELITE_FOUR_PHOEBE, TRAINER_ELITE_FOUR_PHOEBE, TRAINER_ELITE_FOUR_PHOEBE, TRAINER_ELITE_FOUR_PHOEBE, TRAINER_ELITE_FOUR_PHOEBE, EVER_GRANDE_CITY),
    [REMATCH_GLACIA] = REMATCH(TRAINER_ELITE_FOUR_GLACIA, TRAINER_ELITE_FOUR_GLACIA, TRAINER_ELITE_FOUR_GLACIA, TRAINER_ELITE_FOUR_GLACIA, TRAINER_ELITE_FOUR_GLACIA, EVER_GRANDE_CITY),
    [REMATCH_DRAKE] = REMATCH(TRAINER_ELITE_FOUR_DRAKE, TRAINER_ELITE_FOUR_DRAKE, TRAINER_ELITE_FOUR_DRAKE, TRAINER_ELITE_FOUR_DRAKE, TRAINER_ELITE_FOUR_DRAKE, EVER_GRANDE_CITY),
    [REMATCH_WALLACE] = REMATCH(TRAINER_STEVEN, TRAINER_STEVEN, TRAINER_STEVEN, TRAINER_STEVEN, TRAINER_STEVEN, EVER_GRANDE_CITY),
};
...
static void CB2_EndScriptedWildBattle(void)
{
    ...
    if (IsPlayerDefeated(gBattleOutcome) == TRUE)
    {
        ...
        else if (gMapHeader.mapLayoutId == LAYOUT_PACIFIDLOG_CAVE_1F || gMapHeader.mapLayoutId == LAYOUT_METEOR_FALLS_1F_1R) //Marshadow, Deoxys
            SetMainCallback2(CB2_ReturnToFieldContinueScriptPlayMapMusic);
        else
            SetMainCallback2(CB2_WhiteOut);
    }
...
{
    const struct TrainerParty *party;
    u8 i;
    u8 sum;
    u8 partyCount;
    u32 count = numMons;

    if (gSaveBlock2Ptr->newGameOptions.tx_Challenges_AI > AI_MODE_SIMPLE && gTrainers[opponentId].hardparty.HardParty != NULL)
    {
        party = gTrainers[opponentId].hardparty.HardParty;
        partyCount = gTrainers[opponentId].hardpartySize;
    }
    else
    {
        party = gTrainers[opponentId].party.NormalParty;
        partyCount = gTrainers[opponentId].partySize;
    }
...
u8 GetTrainerBattleTransition(void)
{
    ...
    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_ELITE_FOUR)
    {
        ...
        if (gTrainerBattleOpponent_A == TRAINER_ELITE_FOUR_GLACIA)
            return B_TRANSITION_GLACIA;
        if (gTrainerBattleOpponent_A == TRAINER_ELITE_FOUR_DRAKE)
    ...
    if (gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_TEAM_GALACTIC)
    ...
    if (gSaveBlock2Ptr->newGameOptions.tx_Mechanics_Doubles)
        minPartyCount = 1;
    else if (gTrainers[gTrainerBattleOpponent_A].doubleBattle == TRUE)
...
static void CB2_GiveStarter(void)
{
    ...
    *GetVarPointer(VAR_STARTER_MON) = gSpecialVar_Result;
    ...
    if (starterMon == SPECIES_CHARMANDER)
    {
        FlagClear(FLAG_RECEIVED_TOTODILE);
        FlagClear(FLAG_RECEIVED_ROWLET);
    }
    else if (starterMon == SPECIES_TOTODILE)
    {
    ...
    {
        gSaveBlock2Ptr->pokemonFollower.partySlotId = gSaveBlock2Ptr->newGameOptions.tx_Mechanics_Followers;
        //POF_BruteForceFollower(); //It will be removed due Prof. Birch movement
    }
    ResetTasks();
    PlayBattleBGM();
...
void BattleSetup_StartTrainerBattle(void)
{
    ...
    else if (IsBenjaminButterfreeBattle())
    {
        SavePlayerParty();
...
static void CB2_EndTrainerBattle(void)
{
    ...
    else if (FlagGet(FLAG_NO_WHITEOUT_BATTLE))
    {
        //Should register Trainer phone or Set flag?
        FlagClear(FLAG_NO_WHITEOUT_BATTLE);
        HealPlayerParty();
        POF_RenableFollower();
        SetMainCallback2(CB2_ReturnToFieldContinueScriptPlayMapMusic);
    }
...
bool8 NuzlockeIsCaptureBlockedBySameSpecies(u16 species) // @Kurausukun
{
    u8 i;
...
void SetChallengesChecks(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (IsOneTypeChallengeActive())
            OneTypeChallengeIsTypeBlocked[i] = IfSpeciesIsBlockedForOneTypeChallenge(species);

        if (IsNuzlockeActive())
        {
            NuzlockeIsMapBlocked[i] = NuzlockeFlagGet(NuzlockeGetCurrentRegionMapSectionId());

            if (gSaveBlock2Ptr->newGameOptions.tx_Nuzlocke_SpeciesClause)
            {
                NuzlockeIsSpeciesAlreadyCaught[i] = NuzlockeIsCaptureBlockedBySameSpecies(species);
            ...
            if (gSaveBlock2Ptr->newGameOptions.tx_Nuzlocke_ShinyClause && IsMonShiny(&gEnemyParty[i]))
            {
                ...
                NuzlockeIsEvoLineAlreadyCaught[i] = FALSE;
            }
        }
    }
}

void ClearChallengesChecks(void)
{
    u8 i;
    ...
    {
        NuzlockeIsMapBlocked[i] = FALSE;
        OneTypeChallengeIsTypeBlocked[i] = FALSE;
        NuzlockeIsSpeciesAlreadyCaught[i] = FALSE;
        NuzlockeIsEvoLineAlreadyCaught[i] = FALSE;
    }
}

bool8 PlayerCouldCatchPokemon(void)
{
    u8 i;
    u16 species;
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        || species == SPECIES_EGG
        || (oneTypeOn && OneTypeChallengeIsTypeBlocked[i])
        || NuzlockeIsMapBlocked[i]
        || NuzlockeIsSpeciesAlreadyCaught[i]
        || NuzlockeIsEvoLineAlreadyCaught[i])
            continue;
